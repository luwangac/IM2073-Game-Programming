/* ==================================================================================
// CameraRig.cs: This script is attached to the CameraRig GameObject. The script
// 1. makes the camera follow the balls in the game world
// 2. finds an average position and zooms appropriately  to show more than one ball
// ==================================================================================		
...
public class CameraRig : MonoBehaviour
{
    ...
    private void Start()
    {
        _ballTrans = _ballManager.GetBallsTransform();
    ...
    private void CalculateAveragePosition()
    {
        ...
        // Loop to sum up all active balls' position
        for (int i = 0; i < _ballTrans.Length; i++)
        {
            // Skip non-active object(s)
            if (!_ballTrans[i].gameObject.activeSelf)
                continue;

            sumPos += _ballTrans[i].position;
            activeBallsCount++;
        }

        // Get the average by dividing (only if number of active balls is not zero)
        if (activeBallsCount > 0)
            mAveragePos = sumPos / activeBallsCount;
    ...
    protected float GetRequiredSize()
    {
        ...
        // Loop through all balls and check which one is closest to the edge of the screen
        foreach (Transform target in _ballTrans)
        {
            // Skip any balls that are not active
            ...
            // Calculate the size required for this ball
